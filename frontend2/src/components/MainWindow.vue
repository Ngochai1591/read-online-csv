  ...
  <div>
    <div>
      <div class="columns">
        <div class="column">
          ...
          <input type="url" class="files-input" v-model="url" />
    ...
    <div class="columns">
      <div
        ...
        v-show="showTable"
        :class="{
          'half-width': showTable && showCharts,
          'full-width': showTable && !showCharts,
        }"
      >
        <div v-if="totalCount">
          <nav class="pagination">
            <ul class="pagination-list">
              <li v-show="currentPage > 0">
                <a class="pagination-link default-btn" @click="showPage(0)"
                  >P1</a
                >
              </li>
              <li v-show="currentPage - 1 > 0">
                <a
                  class="pagination-link default-btn"
                  @click="showPage(currentPage - 1)"
                  >Pre</a
                >
              </li>
              <li>
                <a class="pagination-link is-current active-btn">{{
                  currentRange
                }}</a>
              </li>
              <li v-show="currentPage + 1 < totalPages - 1">
                <a
                  class="pagination-link default-btn"
                  @click="showPage(currentPage + 1)"
                  >Next</a
              ...
              <li v-show="currentPage < totalPages - 1">
                <a
                  class="pagination-link default-btn"
                  @click="showPage(totalPages - 1)"
                  >P{{ totalPages }}</a
                >
              </li>
            </ul>
          ...
          <table class="table is-narrow" v-if="totalCount">
            <thead>
              <tr>
                ...
                <th class="number-cell" v-for="(h, i) in headers" :key="i">
      ...
      >
        <div v-show="rows.length" class="buttons">
          <a class="button" @click="addChart('Simple')"
            ><icon name="plus"></icon>&nbsp;Simple</a
          >
          <a class="button" @click="addChart('XY')"
            ><icon name="plus"></icon>&nbsp;XY</a
          >
          <a class="button" @click="addChart('Parallel Coordinates')"
            ><icon name="plus"></icon>&nbsp;Parallel Coordinates</a
          >
          <a class="button" @click="addChart('PCA')"
            ><icon name="plus"></icon>&nbsp;PCA</a
          >
          <a class="button" @click="addChart('Histogram')"
            ><icon name="plus"></icon>&nbsp;Histogram</a
          >
          <a class="button" @click="addChart('Histograms')"
            ><icon name="plus"></icon>&nbsp;Histograms</a
          >
          <a class="button" @click="addChart('Correlations')"
            ><icon name="plus"></icon>&nbsp;Correlations</a
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    async getDataFromBackEnd(exportLink) {
      const headers = {
      ...
      let result;
      await axios
        .post(backend_url, data, {timeout: 100*1000})
        .then((data) => {
